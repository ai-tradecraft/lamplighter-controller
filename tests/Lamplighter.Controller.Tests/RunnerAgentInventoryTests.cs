...
public sealed class RunnerAgentInventoryTests
{
    ...
    {
        using var handler = new RecordingHandler(HttpStatusCode.OK, """{"healthy":true}""");
        using var httpClient = new HttpClient(handler, disposeHandler: false);
        var probe = new OpenCodeHealthProbe(httpClient);
    ...
    {
        return Path.Combine(Path.GetTempPath(), $"runner_inventory_{Guid.NewGuid():N}");
    }
